...
def convert_annotation(image_id):
    ...
    if not os.path.exists('./train_labels_to_aug/%s.xml' % (image_id)):
        return

    in_file = open('./train_labels_to_aug/%s.xml' % (image_id))

    out_file = open('aug_train_labels/%s_rotate_bb.txt' % (image_id), 'w')
...
for image in os.listdir('./train_data_to_aug/'):
    ...
    image = Image.open("./train_data_to_aug/" + image_id +".jpg", mode="r")
    image = image.convert("RGB")
    objects = parse_annot("./train_labels_to_aug/" + image_id + ".xml")
    boxes = torch.FloatTensor(objects['boxes'])
    labels = torch.LongTensor(objects['labels'])

    new_image = rotate_only_bboxes(image, boxes, 5)
    new_image.save("./aug_train/" +image_id + "_rotate_bb.png")
